...
class Data():
    ...
    def define_dat_modules(self):
        ...
        for paths in self.package_path:
            self.all_module_paths = glob.glob(join(paths, "*.py"))
            for module_path in self.all_module_paths:
                self.module_paths.append(module_path)
        file_ext = -3
        self.module_list = [ basename(f)[:file_ext] for f in self.module_paths if isfile(f) and not f.endswith('__init__.py')]
